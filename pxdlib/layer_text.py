...
class TextLayer(Layer):
    ...
    @property
    def _text(self):
        return PlistFile.from_base64(self.__string_data()["stringNSCodingData"])
    ...
    def text(self) -> FormattedText:
        ...
        if not hasattr(self._text, "NSAttributeInfo"):
            assert len(style_index) == 1, "Multiple styles but no NSAttributeInfo!"
        ...
        for n, i in chunk(style_rle, 2):
            styles.append((text[n_text : n_text + n], style_index[i]))
            n_text += n
    ...
    def raw_text(self, value: str):
        ...
        if self._multiple_styles():
            ...
            raise AttributeError("Cannot currently set raw_text.")
        _t = self._text
    ...
    def text(self, value: FormattedText):
        raise AttributeError("Cannot set currently set text.")
