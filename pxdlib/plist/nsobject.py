...
class PlistDerefList(NSBaseObject, list):
    ...
    def __str__(self):
        return "[{}]".format(", ".join([str(self._deref(v)) for v in self]))

    def __repr__(self):
        return "<PlistDerefList: [{}]>".format(
            ", ".join(self._repr_child(v) for v in self)
        )
...
class NSObject(NSBaseObject, dict):
    ...
    def __str__(self):
        return "{{{}}}".format(
            ", ".join([f"{k}: {self._parent._deref(v)}" for k, v in self.items()])
        )
    ...
    def __repr__(self):
        ...
        if "$class" in keys:
            keys.remove("$class")
        return "<{} with keys {}>".format(classname(self) or "dict", ", ".join(keys))
