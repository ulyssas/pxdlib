...
class NSArray(NSObject):
    ...
    def __repr__(self):
        return "<NSArray: [{}]>".format(", ".join(self._repr_child(i) for i in self))
...
class NSDictionary(NSObject):
    ...
    def keys(self):
        return NSDictionary._deref(self, NSObject.__getitem__(self, "NS.keys"))

    def values(self):
        return NSDictionary._deref(self, NSObject.__getitem__(self, "NS.objects"))
...
class NSStringOrData(NSObject):
    ...
    _TYPE_PROPERTIES = {str: "NS.string", bytes: "NS.data"}
    ...
    def value(self, new_value: bytes | str):
        if not self.mutable:
            raise AttributeError("Cannot change value of a {}".format(self.classname))

        if not isinstance(new_value, self.type):
            raise AttributeError(
                "{} can only take type {}".format(self.classname, self.type.__name__)
            )
    ...
    def __repr__(self):
        return "<{}: {!r}>".format(self.classname, self.value)
