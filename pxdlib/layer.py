...
from .enums import BlendMode, LayerFlag, LayerTag
...
class Layer:
    ...
    def _info(self, key, default=None):
        ...
        value = self.pxd._db.execute(
            f"select value from layer_info where layer_id = ? and key = ?;",
            (self._id, key),
        ).fetchone()
        if value is None:
            return default
...
_LAYER_TYPES = {1: RasterLayer, 2: TextLayer, 3: VectorLayer, 4: GroupLayer}
