...
class Layer:
    ...
    def parent(self, val: "Layer"):
        ...
        if self.is_mask:
            raise MaskError(
                "Cannot set mask's parent. Try setting `layer.mask = mask` instead."
            )
        ...
        if val.pxd is self.pxd:
            # intra-PXD
            self.pxd._db.execute(
                "update document_layers set parent_identifier = ?where id = ?;",
                (uuid, self._id),
            )
        else:
            # inter-PXD
    ...
    def delete(self):
        ...
        self.pxd._db.execute(f"delete from layer_info where layer_id = {ID};")
        self.pxd._db.execute(f"delete from layer_tiles where layer_id = {ID};")
        self.pxd._db.execute(f"delete from document_layers where id = {ID};")
    ...
    def _copyto(self, parent, asmask, keep_index):
        if self is parent or self._contains(parent):
            raise ValueError("Cannot copy layer into child.")
        if isinstance(parent, Layer):
        ...
        if keep_index:
            (index_at_parent,) = self.pxd._db.execute(
                f"select index_at_parent from document_layers where id = {self._id}"
            ).fetchone()

        ID = self._new_entry(parent, type(self), index_at_parent)

        info = self.pxd._db.execute(
            f"select key, value from layer_info where layer_id = {self._id};",
        ).fetchall()
        for k, v in info:
            destpxd._db.execute(
                "insert into layer_info (layer_id, key, value) values (?, ?, ?)",
                (ID, k, v),
            )
    ...
    def _info_keys(self) -> list[str]:
        ...
        keys = self.pxd._db.execute(
            "select key from layer_info where layer_id = ? ;", (self._id,)
        ).fetchall()
    ...
    def _info(self, key: str, default: Optional[bytes] = None) -> Optional[bytes]:
        ...
        value = self.pxd._db.execute(
            "select value from layer_info where layer_id = ? and key = ?;",
            (self._id, key),
        ).fetchone()
        if value is None:
            return default
    ...
    def _setinfo(self, key, data, create=False):
        ...
        if create:
            ...
            self.pxd._db.execute(
                "update layer_info set value = ? where layer_id = ? and key = ?",
                (data, self._id, key),
            )
    ...
    def is_mask(self, masked: bool):
        not_group = isinstance(self.parent, Layer) and not isinstance(
            self.parent, GroupLayer
        )
    ...
    def adjusts(self):
        ...
        if data:
            data = verb(json.loads(data.decode()), self.COLOR_ADJUSTMENT_STRUCT)
        return ColorAdjustments(self, data)

    @adjusts.setter
    def adjusts(self, val):
        if not isinstance(val, ColorAdjustments):
            raise TypeError("color_adjustments should be ColorAdjustments.")
        data = json.dumps([self.COLOR_ADJUSTMENT_STRUCT, val._data]).encode()
...
class GroupLayer(Layer):
    ...
    def children(self) -> list[Layer]:
        ...
        excluding any masks.
        """
        return [layer for layer in self.pxd._layers(self) if not layer.is_mask]
